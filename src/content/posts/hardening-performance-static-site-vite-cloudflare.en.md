...
An editorial site should be readable outside the UI. RSS is simple, open, and ideal for technical writing. The build generates `blog.xml` and `til.xml` from Markdown metadata. The feed is not manually edited; it is an artifact of the same content source used by the site.
